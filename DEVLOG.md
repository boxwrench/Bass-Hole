...
### 2026-01-16 - Display Color Fix Milestone ✅

**What was done:**

- Fixed sprite color inversion issue through systematic parameter analysis
- Identified and resolved byte-swapping conflict between UI and sprites
- Created reference build with correct colors (washed out but accurate hue/saturation)

**Root cause identified:**

- `DISPLAY_INVERT=true` + manual byte swap was correct for old config
- Changed `DISPLAY_INVERT=false` to fix UI colors
- This broke sprites (double byte-swap)
- Solution: `DISPLAY_INVERT=false` + `setSwapBytes(true)` + remove manual swap

**Changes made:**

- `graphics.cpp:40` - Changed `DISPLAY_INVERT` from `true` to `false`
- `graphics.cpp:82` - Added `tft.setSwapBytes(true)` in `gfxInit()`
- `graphics.cpp:507,522` - Removed manual byte swap `(pixel >> 8) | (pixel << 8)` from `drawPixel` calls

**Current status:**

- ✅ UI colors: CORRECT (gold coins, green button, white text)
- ✅ Sprite colors: CORRECT HUE (right colors, not inverted)
- ⚠️ Sprite appearance: WASHED OUT (low saturation/contrast)
- ✅ Ghosting: FIXED (clean movement)
- ✅ Performance: 30 FPS maintained

**All parameters affecting sprite colors (for reference):**
| Parameter | Location | Setting | Effect |
|-----------|----------|---------|--------|
| `ILI9341_DRIVER` | platformio.ini:16 | ✅ Enabled | Correct driver |
| `TFT_RGB_ORDER` | platformio.ini:27 | `1` (RGB) | Correct |
| `DISPLAY_INVERT` | graphics.cpp:40 | `false` | Fixed UI |
| `tft.setSwapBytes()` | graphics.cpp:82 | `true` | PROGMEM byte order |
| Manual byte swap | graphics.cpp | Removed | Was double-swapping |
| PNG→RGB565 conversion | tools/img2code.py:32 | ✅ Correct | Standard RGB565 |

**Next steps:**

- Investigate washed-out appearance (possible causes: RGB565 bit reduction, PNG source quality, color quantization)
- Consider color boost/saturation adjustment in conversion or display settings
- Document milestone build in git

**Blocking issues resolved:**

- Color inversion: FIXED
- Byte swap conflict: FIXED

**New issues identified:**

- Sprites appear washed out (correct color but low vibrancy)
